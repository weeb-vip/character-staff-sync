...
func EventingAnimeStaffNats() error {
	...
	processorInstance = processorInstance.
		...
		AddMiddleware(backoffretry.NewBackoffRetry[staff_processor.Payload](retryDriver, backoffretry.Config{
	...
	retryProcessorInstance := processor.NewProcessor[*epNats.Message, staff_processor.Payload](retryDriver, retrySubject, procInstance.Process).
		AddMiddleware(NewNatsLoggerMiddleware[staff_processor.Payload]().Process).
		AddMiddleware(NewNatsTransformMiddleware[staff_processor.Payload]().Process).
		AddMiddleware(backoffretry.NewBackoffRetry[staff_processor.Payload](retryDriver, backoffretry.Config{
			MaxRetries: maxRetries,
