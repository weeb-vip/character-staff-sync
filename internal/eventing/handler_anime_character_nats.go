...
func EventingAnimeCharacterNats() error {
	...
	procInstance := character_processor.NewCharacterProcessor[*epNats.Message](processorOptions, repo, NatsProducer(ctx, producerDriver, cfg.NatsConfig.ProducerSubject))
	...
	processorInstance = processorInstance.
		...
		AddMiddleware(backoffretry.NewBackoffRetry[character_processor.Payload](retryDriver, backoffretry.Config{
	...
	retryProcessorInstance := processor.NewProcessor[*epNats.Message, character_processor.Payload](retryDriver, retrySubject, procInstance.Process).
		AddMiddleware(NewNatsLoggerMiddleware[character_processor.Payload]().Process).
		AddMiddleware(NewNatsTransformMiddleware[character_processor.Payload]().Process).
		AddMiddleware(backoffretry.NewBackoffRetry[character_processor.Payload](retryDriver, backoffretry.Config{
			MaxRetries: maxRetries,
